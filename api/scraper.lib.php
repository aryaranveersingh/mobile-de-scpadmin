...
//DATACENTER=SMHQ; DATACENTER=SMHQ; _ga=GA1.2.1682853317.1406121281; __utma=23119776.1682853317.1406121281.1406195682.1406278174.4; __utmz=23119776.1406121281.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); __atuvc=15%7C30; _bcvm_vrid_4564446876128637123=5626196178796068830; DATACENTER=SMHQ; _keyn_rum_session_id=44010789; __utmc=23119776; RT=s=1406278323784&r=http%3A%2F%2Fwww.keynote.com%2Fperformance-indexes%2Fbanking-us%2F; _bcvm_vid_4564446876128637123=5626196922681163983; bc_pv_end=5626197746990871315; _dc_gtm_UA-393555-11=1; __utmb=23119776.1.10.1406278174

function curl($url,$host) {

    $ckfile = tempnam("", "CURLCOOKIE");
    $ch = curl_init();
    //curl_setopt($ch, CURLOPT_PROXY, "anonymouse.org"); 
...
function getRandomUserAgent() {
    ...
    return $user_agent[rand(0,5)];
}


function file_get_html($url, $use_include_path = false, $context = false, $offset = -1, $maxLen = -1, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT) {
    // We DO force the tags to be terminated.
    $host = $use_include_path;
    $postdata = $use_include_path[1];
    $context = null;
    $use_include_path = false;
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
    //$contents = file_get_contents($url, $use_include_path, $context, $offset);
    $contents = curl($url,$host);
...
's", true);
    ...
    function clear() {
        ...
        if (isset($this -> parent)) {$this -> parent -> clear();
            unset($this -> parent);
        }
        if (isset($this -> root)) {$this -> root -> clear();
            unset($this -> root);
        }
        unset($this -> doc);
        unset($this -> noise);
    ...
    function loadFile() {$args = func_get_args();
        $this -> load_file($args);
    }
